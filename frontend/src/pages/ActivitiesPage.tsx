...
type ActivityListResponse = Activity[] | {
    results?: Activity[];
};

const toActivityList = (data: ActivityListResponse): Activity[] => {
...
const ActivitiesPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchActivitiesAndBookings = async () => {
            ...
            setLoadError(null);
            try {
                const allActivitiesRes = await axiosInstance.get<ActivityListResponse>('/api/activities/');
                setActivities(toActivityList(allActivitiesRes.data));
                ...
                setActivities([]);
                setLoadError("Impossible de charger les activités. Vérifiez que l'API est accessible.");
            } finally {
                setLoading(false);
            }
        };
